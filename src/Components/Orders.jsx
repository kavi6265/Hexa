        ...
        Object.keys(allUsersData).forEach(userId => {
          ...
          Object.keys(userOrders).forEach(orderId => {
            if (userOrders[orderId] && typeof userOrders[orderId] === 'object') {
              // Only include orders where delivered is false or not set
              if (!userOrders[orderId].delivered) {
    ...
    <div className="orders-containera">
      ...
      {loading ? (
        ...
        <div className="error-containera">
          <i className="bx bx-error-circle" style={{ fontSize: '48px', color: '#e74c3c', marginBottom: '15px' }}></i>
          <p>{error}</p>
          <button onClick={fetchAllOrders}>Try Again</button>
        </div>
      ) : orders.length === 0 ? (
        <div className="empty-ordersa">
          <i className="bx bx-package empty-icona"></i>
          <h2>No Pending Orders Found</h2>
          <p>{isAdminView ? "There are no pending orders to process at the moment." : "You don't have any pending orders. Start shopping to place an order!"}</p>
          {!isAdminView && (
            <button className="shop-now-btna" onClick={() => navigate('/shop')}>
              Shop Now
            </button>
          )}
        </div>
            ...
            <div key={order.orderId} className="order-carda" onClick={() => handleOrderClick(order)}>
              <div className="order-headera">
                <div className="order-infoa">
                  ...
                  <span className="order-datea">{formatDate(order.orderTimestamp)}</span>
                </div>
                <div className="order-statusa">
                  {order.ordered ? (
                    <span className="statusa processinga">Processing</span>
                  ) : (
                    <span className="statusa pendinga">Pending</span>
                  )}
                </div>
              </div>
                  ...
                  <div key={index} className="product-itema">
                    ...
                    <div className="product-detailsa">
                      ...
                      <div className="product-metaa">
                        <span className="product-qtyA">Qty: {product.qty}</span>
                        <span className="product-pricea">₹{product.productamt}</span>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="order-footera">
                ...
                <div className="order-summarya">
                  ...
                  <button className="track-order-btna">
                    <i className="bx bx-right-arrow-alt" style={{ marginRight: '5px' }}></i>
                    View Details
                  </button>
                </div>
              </div>
